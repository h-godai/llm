...
class InferenceEvaluator:
  ...
  # 初期化
  def __init__(self,  model, openapikey = None):
    self.model_ = model
    ...
    self.eval_ai_model_ = "gpt-4o-2024-11-20"

  # 推論の実行
  def inference(self, prompt):
    print(f"\n======= TASK-{self.eval_count_} : {prompt}\n")
    inputtext = f"### 指示\n{prompt}\n### 回答\n"
    inputs = tokenizer(inputtext, #prompt,
                      return_tensors="pt",
                      padding=True,
  ...
  def completion_with_backoff(self, **kwargs):
    return self.ai_client.chat.completions.create(**kwargs)
...
"""
    ...
    try:
        ...
        gpt4score = None
    time.sleep(0.5) # OpenAIのAPIコールに猶予を与えるため
    return gpt4score
  ...
  def write_result(self, pathname = "LLM2024_result.csv", detail_path = "./outputs", to_excel = True, to_csv = False):
    ...
    resultstr=f"{self.model_name_},{self.temperature_},{self.max_tokens_},{self.average_score_},{self.eval_ai_model_},{now},{self.pasttime_:.1f},0\n"
    print(resultstr)
  ...
  def test(model, tokenizer):
    ...
    test = InferenceEvaluator(model, userdata.get('OPENAI_API_KEY'))
